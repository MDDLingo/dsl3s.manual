...
The models generated with MDD3S have been successfully tested with Java versions 6, 7 and 8. Java 5 was never tested but is likely to also be functional. MDD3S generated models are not expected to run on Java versions 4 and earlier.

Eclipse Kepler has been superseded by Eclipse Lua, but the DSL3S plug-in for this later release is not yet available.
...
2. Expand the _Modelling_ item in the plug-in list and select _Acceleo_ and _Papyrus_. Click _Next_ and wait for the dependencies to be calculated. Go through the following menus and accept the Licenses presented.
...
The DSL3S UML profile and the MDD3S code generator are available as a plug-in to Eclipse Kepler. The install process is similar:
 ...
 2. Select _DSL3S_ and click _Next_ ; wait for the dependencies to be calculated and go on through the following menus accepting the License.
...
Unpack it to some handy place; In Debian based systems this can be _/usr/share/java_, but it can be somewhere else. To run models generated with MDD3S these libraries must be in the Java CLASSPATH.
